...
function getheight(x,y,max=12,min=0) {
    let res = 0;
    for (let i=min;i<=max;i++) {res += getp([x,y],i)/(2**(max-i));}
    return -1000*Math.tanh(0.00295*res*2-0.1)+3*res*2-99.5
}
function test8(x,y) {
    var seed = 10000;
    let v = x*(2**12)+y+seed;
...
for (y=mapa[1]+psize;y<mapa[3]-psize;y+=psize*2) {
    ...
    for (x=mapa[0]+psize;x<mapa[2]-psize;x+=psize*2) {
        ...
        value = value*0.02+0.1
        color = [240-value*30,256-value*10,256-value*20]
